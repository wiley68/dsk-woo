...
/**
 * Outputs the responsive popup banner (desktop/mobile via picture + CSS).
 *
 * @since 1.2.2
 * @param string|int $reklama_id Banner variant id from API.
 * @return void
 */
function dskapi_render_popup_banner( $reklama_id ) {
	if ( '' === (string) $reklama_id ) {
		return;
	}

	$reklama_id = sanitize_key( (string) $reklama_id );
	if ( '' === $reklama_id ) {
		return;
	}
	$desktop    = DSKAPI_LIVEURL . '/calculators/assets/img/dsk' . $reklama_id . '.png';
	$mobile     = DSKAPI_LIVEURL . '/calculators/assets/img/dskm' . $reklama_id . '.png';
	?>
	<picture>
		<source media="(max-width: 767px)" srcset="<?php echo esc_url( $mobile ); ?>">
		<img src="<?php echo esc_url( $desktop ); ?>" class="dskapi_header" alt="">
	</picture>
	<?php
}
...
function dskapi_add_meta() {
	if ( is_front_page() ) {
		...
		wp_enqueue_style( 'dskapi_style_rek', DSKAPI_CSS_URI . '/dskapi_rek.css', array(), filemtime( $css_file ), 'all' );
